...
export async function upsertLetter(letter: Partial<Letter> & { id: string }): Promise<void> {
  const { error } = await getSupabase().from("letters").upsert(letter, { onConflict: "id" });
  if (error) throw error;
}
...
export async function getSettings(): Promise<Settings> {
  ...
  if (error) throw error;
  if (!data) {
    await getSupabase().from("settings").insert({ id: "global", disabled_letters: [] });
...
export async function uploadLetterAudio(letterId: string, blob: Blob): Promise<string> {
  const path = `${letterId}/pronunciation.webm`;
  const { error } = await getSupabase().storage
    .from("letter-audio")
    .upload(path, blob, { upsert: true, contentType: "audio/webm" });
  if (error) throw error;
  const { data } = getSupabase().storage.from("letter-audio").getPublicUrl(path);
  return data.publicUrl;
}

export async function uploadExampleImage(letterId: string, slotId: number, blob: Blob): Promise<string> {
  const path = `examples/${letterId}/${slotId}.jpg`;
  const { error } = await getSupabase().storage
    .from("examples")
    .upload(path, blob, { upsert: true, contentType: "image/jpeg" });
  if (error) throw error;
  const { data } = getSupabase().storage.from("examples").getPublicUrl(path);
  return data.publicUrl;
}

export async function uploadExampleAudio(letterId: string, slotId: number, blob: Blob): Promise<string> {
